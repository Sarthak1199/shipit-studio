    ...
    <footer className="border-t border-line bg-bg">
      <div className="container-x grid gap-12 py-16 md:grid-cols-12 md:py-20">
        <div className="md:col-span-5">
          ...
          <a href={site.instagram} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex min-h-[44px] items-center gap-2 text-ink hover:text-pop">
        ...
        <div className="md:col-span-2">
          ...
          <ul className="mt-4 space-y-1">
            {explore.map((l) => (
              <li key={l.href}><Link href={l.href} className="inline-flex min-h-[40px] items-center text-muted hover:text-ink">{l.label}</Link></li>
            ))}
          </ul>
        </div>
        <div className="md:col-span-2">
      ...
      <div className="border-t border-line">
        <div className="container-x flex flex-col gap-2 py-6 font-mono text-[11px] uppercase tracking-[0.18em] text-muted sm:flex-row sm:items-center sm:justify-between">
          <span>All Rights Reserved © 2026 by ShipIt Studio</span>
          <span>Gurugram → everywhere</span>
        </div>
      </div>
    </footer>
  );
