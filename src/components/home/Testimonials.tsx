    ...
    <section className="section border-t border-line">
      <div className="container-x grid gap-10 md:grid-cols-12 md:items-center">
        <Reveal className="md:col-span-5">
          ...
          <Item><h2 className="h-display mt-4 text-[clamp(2.2rem,5vw,3.6rem)]">We build in public.<br /><em>Come watch.</em></h2></Item>
          <Item>
        ...
        <div className="relative h-[380px] sm:h-[340px] md:col-span-7">
          <div aria-hidden className="card absolute inset-x-6 top-6 h-full rotate-[3deg] opacity-50" />
          <div aria-hidden className="card absolute inset-x-3 top-3 h-full rotate-[-2deg] opacity-70" />
          <AnimatePresence mode="popLayout" initial={false}>
            <motion.figure key={i} drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.6}
              onDragEnd={(_, info) => { if (info.offset.x < -60) next(); else if (info.offset.x > 60) prev(); }}
              initial={{ opacity: 0, x: 60, rotate: 4 }} animate={{ opacity: 1, x: 0, rotate: 0 }} exit={{ opacity: 0, x: -60, rotate: -4 }}
              transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
              className="card absolute inset-0 flex cursor-grab touch-pan-y flex-col justify-between p-7 active:cursor-grabbing sm:p-9">
              <span className="font-display text-6xl leading-none text-sage">“</span>
              <blockquote className="font-display text-xl leading-snug sm:text-2xl">{items[i].quote}</blockquote>
              <figcaption className="mt-6 flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-moss font-display text-sm text-bg">{items[i].name[0]}</span>
                <span><span className="block font-semibold">{items[i].name}</span><span className="block text-sm text-muted">{items[i].role}</span></span>
              </figcaption>
            </motion.figure>
          </AnimatePresence>
